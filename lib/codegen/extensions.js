 ...
 * @param prefix Prefix that has to be appended to all origins (optional)
 *
...
function parse(options) {
    ...
    if (options.extensionsWarnOnConflict.toLowerCase() === 'all') {
...
function readExtensionBundle(dir, options) {
    ...
    if (options.extensionsWarnOnConflict.toLowerCase() !== 'none') {
        for (i = 0; i < checks.length; i++) {
            console.log("Warning: " + checks[i].error);
        }
    }
